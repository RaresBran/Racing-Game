...
void Lab2::Init()
{
    ...
    vector<VertexFormat> vertices2
        {
            VertexFormat(glm::vec3(0, 0, 1), glm::vec3(0, 1, 1), glm::vec3(0.7, 0.8, 0.6)),
            VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0, 1, 1), glm::vec3(0.7, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, 1, 0), glm::vec3(0, 1, 1), glm::vec3(0.7, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0, 1, 1), glm::vec3(0.7, 0.8, 0.6))
        };
    ...
    meshes["tetrahedron_A"]->InitFromData(vertices2, indices2);

    // Actually create the mesh from the data
    CreateMesh("tetrahedron_B", vertices2, indices2);
    ...
    meshes["square_A"] = new Mesh("generated square 1");
    meshes["square_A"]->InitFromData(vertices3, indices3);

    // Actually create the mesh from the data
    CreateMesh("square_B", vertices3, indices3);
}
...
void Lab2::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(vertices[0]) * indices.size(), &indices[0], GL_STATIC_DRAW);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["tetrahedron_A"], shaders["VertexColor"], glm::vec3(0, 0.5f, 0));

    // TODO(student): Draw the square
    RenderMesh(meshes["square_A"], shaders["VertexColor"], glm::vec3(1, 1, 1));
